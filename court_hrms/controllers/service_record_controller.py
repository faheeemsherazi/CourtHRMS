    ...
    def create_record(self, data: dict) -> tuple[bool, str, dict | None]:
        try:
            with session_scope() as session:
                ...
                return True, "Service record saved successfully.", self._record_to_dict(record)
        except ValidationError as exc:
            return False, str(exc), None

    def update_record(self, record_id: int, data: dict) -> tuple[bool, str, dict | None]:
        try:
            with session_scope() as session:
                service = ServiceRecordService(session)
                record = service.update_record(record_id, data)
                return True, "Service record updated successfully.", self._record_to_dict(record)
        except ValidationError as exc:
            return False, str(exc), None

    def find_staff(self, personal_number: str) -> tuple[bool, str, dict | None, dict | None]:
        with session_scope() as session:
